...
def get_video_ranks(query_image, feats_path, metadata_path):
    ...
    centers = list(np.load(os.path.join(metadata_path, 'centers.npy'), allow_pickle=True))
...
def main():
    ...
    query_image_path = os.path.join(os.path.expanduser('~'), "test_query.png")
    print(query_image_path)
    query_image = img2vec.get_vec_single(query_image_path).reshape(1, -1)

    x1 = time.time()
    
    video_ranks = get_video_ranks(query_image, os.path.join(root, "data", "feats", "resnet152"), os.path.join(root, "metadata"))
